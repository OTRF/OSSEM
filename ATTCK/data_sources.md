...
|-----------------|----------------|
|	Anti-virus	|	Alerts generated by antivirus solutions that need to be investigated	|
|	API monitoring	|	Logs monitoring API calls on the endpoint	|
|	Authentication logs	|	Logs tracking login activity related to remote services. For example, processes authenticating to other endpoints via WinRM, WMI, etc.	|
|	Binary file metadata	|	Information about files over the wire or locally on an endpoint. For example, information about digital certificates, files being compressed, suspicious utilities, etc	|
|	BIOS	|	Logs providing information about the integrity of existing BIOs (might not be provided by default logs on the endpoints)	|
...
|	Environment variable	|	Logs tracking users checking or changing their HISTCONTROL environment variables. (Lunix/MacOs)	|
...
|	Netflow/Enclave netflow	|	Netflow logs	|
|	Network device logs	|	Network logs from endpoints (src_port, dst_port, etc)	|
...
|	Services	|	Logs about services being installed or highjacked in a system	|
|	SSL/TLS inspection	|	Information about encrypted channels being used by adversaries	|
|	System calls	|	Logs indicating high privileged execution of code in the computer.	|
...
|	Windows event logs	|	Windows event logs used to track user creation, permissions modifications, and even changes to groups. This data source focuses on scheduled tasks, account manipulations, account creation, SID-history Injection and more 	|
...
|	Access Tokens	|	Logs tracking token manipulation. One way can be done via command-line auditing. For example, adversaries using runas-like procedures to impersonate users.	|
...
|	Named Pipes	|	Logs tracking named pipes creation and connection events (Sysmon Event IDs 17 and 18) for possible indicators of infected processes with external modules	|