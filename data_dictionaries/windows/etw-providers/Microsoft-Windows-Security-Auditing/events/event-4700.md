...
|---|---|---|---|---|
|user_sid|SubjectUserSid|SID|SID of account that requested the "enable scheduled task" operation|`SOMEDOMAIN\SomeUser`|
|user_name|SubjectUserName|UnicodeString|the name of the account that requested the "enable scheduled task" operation.|`SomeUser`|
|user_domain|SubjectDomainName|UnicodeString|subject's domain or computer name.|`SOMEDOMAIN`|
|user_logon_id|SubjectLogonId|HexInt64|hexadecimal value that can help you correlate this event with recent events that might contain the same Logon ID|`0x364fb`|
|scheduled_task_name|TaskName|UnicodeString|enabled scheduled task name.|`\SomeTask`|
