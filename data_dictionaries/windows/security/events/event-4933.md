...
|naming_context|NamingContext|TBD|string|naming context to replicate.|CN=Schema,CN=Configuration,DC=contoso,DC=local|
|options|Options|TBD|integer|decimal value of DRS Options.|2147483733|
|session_id|SessionID|TBD|integer|unique identifier of replication session. Using this field you can find "4932: Synchronization of a replica of an Active Directory naming context has begun." and "4933: Synchronization of a replica of an Active Directory naming context has ended." events for the same session.|40|
|end_usn|EndUSN|TBD|integer|Naming Context's USN number after replication ends.|20869|
|status_code|StatusCode|TBD|integer|if there are no issues or errors, the status code will be "0". If an error happened, you will receive Failure event and Status Code will not be equal to "0".|1722|
